...
import type { TemplateMarkNode, TemplateMarkDocument } from '../types/TemplateMark';
...
export function tmNodeToTipTap(node: TemplateMarkNode): JSONContent | JSONContent[] | null {
  ...
  if (cls === 'org.accordproject.commonmark@0.5.0.Image') {
    const n = node as { $class: string; destination?: string; title?: string; nodes?: TemplateMarkNode[] };
    const altText = n.nodes?.find((c) => c.$class === 'org.accordproject.commonmark@0.5.0.Text') as { text?: string } | undefined;
    return {
      type: 'image',
      attrs: {
...
export function tipTapNodeToTM(node: JSONContent): TemplateMarkNode | TemplateMarkNode[] | null {
  ...
  if (type === 'text') {
    ...
    let result: TemplateMarkNode = { $class: 'org.accordproject.commonmark@0.5.0.Text', text };
    for (const mark of marks) {
      if (mark.type === 'bold') {
        ...
        result = { $class: 'org.accordproject.commonmark@0.5.0.Code', nodes: [result] } as TemplateMarkNode;
  ...
  if (type === 'image') {
    const altTextNode = node.attrs?.alt
      ? [{ $class: 'org.accordproject.commonmark@0.5.0.Text' as const, text: node.attrs.alt }]
      : [];
    return {
