...
export const TemplateConditionalNodeView = ({ node, updateAttributes, }) => {
    const { name, whenTrueJson, whenFalseJson } = node.attrs;
    const [modalOpen, setModalOpen] = useState(false);
    const openModal = useCallback((e) => {
        e.stopPropagation();
        setModalOpen(true);
    }, []);
    const handleSave = useCallback((trueMd, falseMd) => {
        updateAttributes({
    ...
    return (_jsxs(NodeViewWrapper, { as: "span", children: [_jsxs("span", { contentEditable: false, style: wrapStyle, children: [_jsxs("span", { style: pillStyle('#bee3f8', '#2a4365', '#63b3ed'), children: [_jsx("span", { style: labelStyle, children: "if: " }), _jsx("strong", { children: name })] }), _jsxs("span", { style: branchStyle('#ebf8ff'), children: [_jsx("span", { style: branchLabel, children: "true" }), _jsx("span", { style: previewStyle, children: truePreview || '(empty)' })] }), _jsx("span", { style: dividerStyle, children: "|" }), _jsxs("span", { style: branchStyle('#fff5f5'), children: [_jsx("span", { style: branchLabel, children: "false" }), _jsx("span", { style: previewStyle, children: falsePreview || '(empty)' })] }), _jsx("button", { onClick: openModal, contentEditable: false, style: editBtnStyle, title: "Edit branches", children: "\u270F Edit" })] }), modalOpen &&
                ReactDOM.createPortal(_jsx(BranchModal, { title: `Edit conditional: ${name}`, branches: [
                    ...
                    ], onSave: ([trueMd, falseMd]) => handleSave(trueMd, falseMd), onClose: () => setModalOpen(false) }), document.body)] }));
};
