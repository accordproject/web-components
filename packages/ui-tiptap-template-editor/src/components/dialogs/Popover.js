 ...
 */
export const Popover = ({ anchor, onClose, children, width = 220 }) => {
    const rect = anchor?.getBoundingClientRect();
    const top = rect ? rect.bottom + window.scrollY + 4 : 100;
    const left = rect ? rect.left + window.scrollX : 100;
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "ap-popover-backdrop", onClick: onClose }), _jsx("div", { className: "ap-popover", style: { top, left, minWidth: width }, onClick: (e) => e.stopPropagation(), children: children })] }));
};
// ── CSS Class Names ───────────────────────────────────────────────────────────
