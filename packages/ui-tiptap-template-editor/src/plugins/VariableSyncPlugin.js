 ...
 */
function collectVariablesByName(doc) {
    ...
    doc.forEach((topNode, topPos) => {
        ...
        const walk = (node, pos) => {
            if (node.type.name === 'variable' || node.type.name === 'formattedVariable') {
                ...
                const entry = { pos, text: node.textContent, clausePos };
 ...
 */
function collectVariableTexts(doc) {
    const result = new Map();
    const walk = (node, pos) => {
        if (node.type.name === 'variable' || node.type.name === 'formattedVariable') {
 ...
 */
export function createVariableSyncPlugin() {
    return new Plugin({
        ...
        appendTransaction(transactions, oldState, newState) {
            ...
            if (!transactions.some(tr => tr.docChanged))
                return null;
            if (transactions.some(tr => tr.getMeta('variableSync')))
                return null;
