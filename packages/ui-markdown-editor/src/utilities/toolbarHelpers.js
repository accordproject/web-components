...
import { LIST_ITEM, BLOCK_QUOTE, LIST_TYPES, PARAGRAPH } from "./schema";
...
export const toggleBlock = (editor, format) => {
  ...
  Transforms.unwrapNodes(editor, {
    match: (n) => isQuote(n.type),
    split: true,
  });
  Transforms.unwrapNodes(editor, {
    match: (n) => isListItem(n.type),
    split: true,
  });
  Transforms.unwrapNodes(editor, {
  ...
  if (!isActive) {
    const formattedBlock = {
      type: format,
      children: [],
      data: isQuote(format) ? {} : { tight: true },
    };
    ...
    if (isList(format)) {
      ...
      const focus = editor.selection.focus.path
        ...
        .concat(
          0,
          editor.selection.focus.path[editor.selection.focus.path.length - 1]
        );
