...
export const matchCases = (editor, currentLine) => {
    ...
    const currentRange = {
      ...
      focus: {
        ...
        offset: editor.selection.focus.offset - (textToInsert.length + 1 + markdownCharacters)
      }
    }
  ...
  const matchCodeInline = () => {
    ...
    const codeText = codeInlineMatchCase[0].trim().replace(new RegExp(codeInlineMatchCase[1], "g"), "");

    insertFormattedInlineText(codeText, {
      code: true
    }, 2);
  ...
  const matchBoldAndItalic = () => {
    ...
    if (lastChar === "*" || lastChar === "_") {
      if ((boldAndItalicMatchCase = prevTextFromSpace.match(/\s?(\*\*\*|___)((?!\1).)+?\1$/m))) {
				// ***[bold + italic]***, ___[bold + italic]___
				const reg = boldAndItalicMatchCase[1] === "***" ? /\*\*\*/ : boldAndItalicMatchCase[1];
        const boldAndItalicText = boldAndItalicMatchCase[0].trim().replace(new RegExp(reg, "g"), "");
        insertFormattedInlineText(boldAndItalicText, {
          bold: true,
          italic: true
        }, 6)
      } else if ((boldMatchCase = prevTextFromSpace.match(/\s?(\*\*|__)((?!\1).)+?\1$/m))) {
    		// **bold**, __bold__
				const reg = boldMatchCase[1] === "**" ? /\*\*/ : boldMatchCase[1];
        const boldText = boldMatchCase[0].replace(new RegExp(reg, "g"), "");
        insertFormattedInlineText(boldText, {
          bold: true
        }, 4)
      } else if ((italicMatchCase = prevTextFromSpace.match(/\s?(\*|_)((?!\1).)+?\1$/m))) {
				// *italic*, _italic_
        const reg = italicMatchCase[1] === "*" ? /\*/ : italicMatchCase[1];
        const italicText = italicMatchCase[0].replace(new RegExp(reg, "g"), "");
        insertFormattedInlineText(italicText, {
          italic: true
        }, 2)
      }
